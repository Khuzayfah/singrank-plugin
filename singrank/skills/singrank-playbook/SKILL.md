...
| `llm_local.py "<prompt>" [--file f] [--check\|--up\|--down]` | LOCAL model (Ollama qwen2.5:14b on the RTX 5080, localhost:11434) — token-free heavy lifting: summarize scraped pages, first-pass rewrite drafts, bulk classification, title variants. Output = draft input ONLY; always passes the same QC gates. **Lifecycle: ON-DEMAND, never always-on** — login autostart is disabled; the server auto-starts when a call needs it; END EVERY WORK SESSION with `llm_local.py --down` (unloads VRAM + stops the server) | When the task is bulk/mechanical language work — save Claude tokens for strategy, final writing, and QC |
| `smart_scrape.py --topic "<t>" --urls <seeds...> [--depth 1]` | GPU-powered deep scrape: parallel fetch → embedding relevance filter (nomic-embed-text — irrelevant pages/links dropped before they cost anything) → local-LLM extraction per page (citable facts + verbatim sentence + numbers + entities + source URL) → distilled PACK.md + facts.jsonl. Fetching is network-bound; the GPU powers the UNDERSTANDING layer. Every fact still passes `web_research.py verify` before entering an article | Bulk research: many sources for one topic (deep-research Phase 3 at scale) — raw HTML never touches Claude's context |
| `prospect_hunter.py qualify --niche "<n>" --urls <sites...>` | Qualified B2B lead gen: per candidate business, runs the live SEO audit + extracts PUBLIC business contact (tech-noise filtered) + scores the lead (SEO-weak-and-reachable = HOT). Ranked LEADS.md + leads.jsonl, each lead carrying real audit pitch hooks. PDPA-safe: public business contact only, respects robots.txt, no personal harvesting | Agency growth — find SG/ID businesses that need SEO (singrank-prospecting skill / /seo-prospect) |
| `originality_check.py <draft> --sources <facts.jsonl\|PACK.md>` | Anti-plagiarism guard: flags LIFTED (8+ word run copied from a source = P0) and ECHO (paraphrase too close = P1) vs the scraped source material. Enforces "facts borrowed, words ours". Checks WORDING (verify checks FACTS — both required) | Before publishing any article that used external sources (singrank-synthesis skill; qc Step 1b) |
...
| "cari klien" / "cari prospek" / "lead generation" / "siapa yang butuh SEO" | **singrank-prospecting** (or /seo-prospect) — qualified audit-proven B2B leads |
| "olah hasil scraping jadi tulisan" / "tulis dari pemahaman" / "jangan copy paste" / "sintesis" | **singrank-synthesis** (+ originality_check.py gate) |
